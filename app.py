...
SCENARIO_IMPACT_MAP = {
    ...
    "[FW] 電源障害：両系": ImpactLevel.DEGRADED_MID,
...
def _make_alarms_legacy(topology: dict, selected_scenario: str):
    ...
    if target_device_id:
        if "電源" in selected_scenario:
            alarms.append(Alarm(target_device_id, "Power Supply Failed", "CRITICAL"))
...
def _status_from_alarms(selected_scenario: str, alarms) -> str:
    ...
    if impact_level >= ImpactLevel.COMPLETE_OUTAGE: return "停止"
    elif impact_level >= ImpactLevel.CRITICAL: return "要対応"
    elif impact_level >= ImpactLevel.DEGRADED_MID:
        severities = [str(getattr(a, "severity", "")).upper() for a in alarms]
        if any(s == "CRITICAL" for s in severities): return "要対応"
        return "注意"
    elif impact_level >= ImpactLevel.DOWNSTREAM: return "注意"
    else: return "正常"
...
def _build_company_rows(selected_scenario: str):
    ...
    try:
        ...
        all_scopes = [("A", "default"), ("B", "default")] # Fallback
    ...
    for tenant_id, network_id in all_scopes:
        ...
        # メンテナンス中はステータスを強制上書き等はここで行うが今回は表示のみ
...
def _render_all_companies_board(selected_scenario: str, df_height: int = 220):
    ...
    # 2. パステルカラー・ツリーマップ (独立キューブ風 & クリック対応)
    try:
        import plotly.express as px
        
        color_map = {
            "停止": "#FFCDD2",    # Pastel Red
            "要対応": "#FFE0B2",  # Pastel Orange
            "注意": "#FFF9C4",    # Pastel Yellow
            "正常": "#C8E6C9"     # Pastel Green
        }
        
        tree_data = []
        for r in rows:
            weight = r['alarm_count'] + (20 if r['status'] == '停止' else 1)
            # ラベル (件数を強調)
            display_label = f"<b>{r['company_network']}</b><br>({r['alarm_count']}件)"
            ...
            tree_data.append({
                ...
                "Tenant": r["tenant"],
                "Network": r["network"],
                "DisplayLabel": display_label # カスタムデータ用
            })
        ...
        if not df_tree.empty:
            fig = px.treemap(
                df_tree,
                path=['Status', 'DisplayLabel'],
                values='Weight',
                color='Status',
                color_discrete_map=color_map,
                hover_data={'Alarms': True, 'Weight': False, 'Status': False},
                custom_data=['Tenant', 'Network'] # クリック時に抽出するデータ
            )
            ...
            fig.update_traces(
                ...
                marker=dict(
                    line=dict(width=6, color='#FFFFFF'), # 太い白枠で分離
                    pad=dict(b=5, l=5, r=5, t=5)
                ),
                root_color="rgba(0,0,0,0)"
            )
            
            # 【重要】ダブルトリガー1: ツリーマップからの選択
            event_map = st.plotly_chart(
                fig, 
                use_container_width=True, 
                on_select="rerun", # クリックでリロード
                selection_mode="points",
                key="treemap_select"
            )

            # 選択イベントのハンドリング
            if event_map and event_map.selection["points"]:
                point = event_map.selection["points"][0]
                # customdataに[Tenant, Network]が入っている
                if "customdata" in point:
                    selected_tenant = point["customdata"][0]
                    selected_network = point["customdata"][1]
                    # セッション更新して再描画
                    st.session_state.selected_scope = {"tenant": selected_tenant, "network": selected_network}
                    st.rerun()

    except ImportError:
        st.error("Plotly package is required for the dashboard.")

    # 3. トリアージ・グリッド (ダブルトリガー2: リストからの選択)
    ...
    if high_priority_rows:
        ...
        event_h = st.dataframe(
            ...
            selection_mode="single-row", on_select="rerun", key="grid_high"
        )
        # リスト選択イベントのハンドリング
        if len(event_h.selection.rows) > 0:
            sel = df_high.iloc[event_h.selection.rows[0]]
    ...
    if warn_rows:
        with st.expander(f"⚠️ Watch List ({len(warn_rows)}件)", expanded=False):
            ...
            event_w = st.dataframe(
                ...
                use_container_width=True, hide_index=True, 
                selection_mode="single-row", on_select="rerun", key="grid_warn"
            )
            # リスト選択イベントのハンドリング
            if len(event_w.selection.rows) > 0:
                sel = df_warn.iloc[event_w.selection.rows[0]]
                st.session_state.selected_scope = {"tenant": sel['raw_tenant'], "network": sel['raw_network']}
                st.rerun()
...
# ======================================================================================
# 下段：AIOps インシデント・コックピット (選択された会社を表示)
# ======================================================================================
