...
def _get_impact_display(cand: dict, scope_status: str) -> str:
    """影響度を適切に表示（状態 + 数値の併記）"""
    prob_pct = f"{cand['prob']*100:.0f}%"
    
    if scope_status == "停止":
        return f"🔴 サービス停止(100%)"
    elif cand['prob'] > 0.8:
        return f"🔴 CRITICAL ({prob_pct})"
    elif cand['prob'] > 0.6:
        return f"🟡 WARNING ({prob_pct})"
    else:
        return f"⚪ NORMAL ({prob_pct})"
...
event = st.dataframe(
    df,
    column_order=["順位", "ステータス", "根本原因候補", "影響度", "推奨アクション"],
    column_config={
        "影響度": st.column_config.TextColumn(
            "影響度",
            help="サービス影響度とリスクスコア",
            width="medium"
        )
...
with col_chat:
        ...
        if "generated_report" not in st.session_state or st.session_state.generated_report is None:
            ...
            if api_key and selected_scenario != "正常稼働":
                if st.button("📝 詳細レポートを作成 (Generate Report)"):
                    ...
                    - 影響度スコア: {"サービス停止(100%)" if scope_status=="停止" else f"{cand['prob']*100:.0f}%"}
    ...
    if selected_incident_candidate and selected_incident_candidate["prob"] > 0.6:
        ...
        <div style="background-color:#e8f5e9;padding:10px;border-radius:5px;border:1px solid #4caf50;color:#2e7d32;margin-bottom:10px;">
            ...
            (影響度スコア: <span style="font-size:1.2em;font-weight:bold;">{selected_incident_candidate['prob']*100:.0f}%</span>)
        </div>
        ...
        if selected_incident_candidate:
            if scope_status == "停止":
                score_display = "サービス停止(100%)"
            else:
                score_display = f"{selected_incident_candidate['prob']*100:.0f}%"
            
            st.warning(f"""
            ⚠️ **自動修復はロックされています**
            現在選択されているインシデントの影響度スコアは **{score_display}** です。
