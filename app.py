...
def _render_all_companies_board(selected_scenario: str, df_height: int = 220):
    ...
    st.markdown("<small>色の濃さ = アラーム件数 | クリックで詳細表示</small>", unsafe_allow_html=True)
    
    # 会社をアラーム数順にソート
    rows_sorted = sorted(rows, key=lambda x: (-x['alarm_count'], x['company_network']))
    ...
    # グリッド表示（4列）
    cols_per_row = 4
    num_rows = (len(rows_sorted) + cols_per_row - 1) // cols_per_row
    ...
    for row_idx in range(num_rows):
        ...
        for col_idx in range(cols_per_row):
            ...
            if item_idx >= len(rows_sorted):
                break
                
            r = rows_sorted[item_idx]
            ...
            # アラーム数に応じて色の濃さを調整
            if r['alarm_count'] > 0:
                # アラーム数が多いほど濃くなる（最大50件で飽和）
                alarm_intensity = min(r['alarm_count'] / 50, 1.0)
                # 元の強度と組み合わせ
                final_intensity = (intensity * 0.5) + (alarm_intensity * 0.5)
                # 背景色をより濃くする
                if r['status'] == '停止':
                    bg_color = f"rgba(239, 83, 80, {0.3 + final_intensity * 0.7})"
                elif r['status'] == '要対応':
                    bg_color = f"rgba(251, 140, 0, {0.3 + final_intensity * 0.7})"
                elif r['status'] == '注意':
                    bg_color = f"rgba(251, 192, 45, {0.3 + final_intensity * 0.7})"
            
            # デルタ表示
            delta_html = ""
            if r['delta'] is not None:
                if r['delta'] > 0:
                    delta_html = f'<div class="heat-delta" style="color: red;">▲ +{r["delta"]}</div>'
                elif r['delta'] < 0:
                    delta_html = f'<div class="heat-delta" style="color: green;">▼ {r["delta"]}</div>'
            
            # メンテナンスバッジ
            maint_badge = ""
            if r['maintenance']:
                maint_badge = '<div style="position: absolute; top: 10px; left: 10px;">🛠️</div>'
            ...
            with cols[col_idx]:
                ...
                <div class="heat-cell" style="background: {bg_color};">
                    ...
                    {maint_badge}
                    <div class="heat-title" style="color: {text_color};">{r['company_network']}</div>
                    <div class="heat-count" style="color: {text_color};">{r['alarm_count']}</div>
                    <div style="color: {text_color}; font-size: 0.9em;">アラーム</div>
                    {delta_html}
                </div>
                """
                ...
                # クリック処理（透明ボタン）
                if st.button("", key=f"heat_{r['tenant']}_{r['network']}", 
                            help=f"{r['company_network']}の詳細を表示"):
